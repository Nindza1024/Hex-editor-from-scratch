...
Строка ```//document.getElementById("outputDann").appendChild(createHexRedactorLine(dannArray.slice( i, i+(position % 16) ),i));``` временно заккоментрирована до написания функции ```createHexRedactorLine(lengthLine,number)``` в которую подается массив из 1-16 элементов (переменная lengthLine) и позиция этих элементов. Именно в этой функции генерироваться окна для вывода на экран пользователя.
